...
def nuniroot(f, low_val: float = 0, high_val: float = 1, max_length: int = 100) -> float:
    ...
    if min(f_output) * max(f_output) > 0:
        raise ValueError("The specified parameters do not yield valid results. Please try to supply a different interval, e.g., using interval=c(0,1), for your parameter.")
    else:
